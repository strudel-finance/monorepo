...
contract MasterChef is Ownable {
    ...
    function pendingStrudel(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 strudelReward = multiplier
                ...
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accStrudelPerShare = accStrudelPerShare.add(
                strudelReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accStrudelPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accStrudelPerShare).div(1e12).sub(
            user.rewardDebt
        );
        safeStrudelTransfer(msg.sender, pending);
