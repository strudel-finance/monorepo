...
// Use Harmony one instead
// import "./summa-tx/IRelay.sol";
import "./harmony-relay/IRelay.sol";
...
contract VbtcToken is FlashERC20, ERC20CappedUpgradeSafe {
  ...
  ///                  Height is used for the Harmony relayer
  /// @param _height   header
  ...
  function proofOpReturnAndMint(
    ...
    bytes4 _locktime,
    uint256 _index,
    uint32 _crossingOutputIndex, // Height in Harmony layer
    bytes calldata _vin,
  ...
  function _provideProof(
    ...
    bytes memory _header, // can this be reused?
    bytes memory _proof, // can this be reused?
    
    bytes4 _version,
    bytes4 _locktime,
    uint256 _index, // can this be reused?
    uint32 _crossingOutputIndex, // Currently 0 for Harmony?
    ...
    require(relay.verifyTx(
      _height,
      _index,
      txId, // This can be calculated from the server already
      _header,
      _proof,
      
      6,
      false
    ), "Bitcoin transaction details submitted are not correct");
