...
const getBlockHeader = require("./utils/blockheader");
const RelayAbi = require("./abi/RelayAbi");
const vBtcContract = require("./abi/VBTCContract");

module.exports = class RelayerHandler {
  constructor(provider, bclient, wallet) {
    this.provider = provider;
    this.bclient = bclient;
    this.wallet = wallet;
  }

  async updateBlockHeader() {
    await this.bclient.getHeader(blockhash);
  }
  ...
  async addSig(accountAddr, sig) {
    ...
    //   0x87 - OP_EQUAL
    const p2fshBuf = Buffer.from(
      `0xa914${accountAddr.replace("0x", "")}87`,
      "hex"
    );
    // check sig
    let addr = ethers.utils.verifyMessage(
      p2fshBuf,
      `${sig.r}${sig.s.replace("0x", "")}${sig.v.toString(16)}`
    );
  ...
  async addBtcTx(txHash, txData) {
    ...
    const tx = Transaction.fromBuffer(
      Buffer.from(txData.replace("0x", ""), "hex")
    );
    const txId = tx.getId();
    ...
    tx.outs.forEach((out, i) => {
      if (
        out.script.readUInt8(0) === opcodes.OP_RETURN &&
        out.script.length == 25
      ) {
        count++;
        index = i;
      }
    ...
    if (count < 1) {
      throw new Errors.BadRequest(
        `${txId} has no OP_RETURN output matching protocol.`
      );
    }
    if (count > 1) {
      throw new Errors.ServerError(
        `multiple OP_RETURN outputs not implemented.`
      );
    }
    if (tx.outs[index].value < 1) {
      throw new Errors.ServerError(`output has 0 value.`);
    }
    const walletAddress = `0x${tx.outs[index].script
      .slice(5, 25)
      .toString("hex")}`;
    await this.db.setPaymentOutput(
      txId,
      index,
      walletAddress,
      `${tx.outs[index].value}`
  ...
  async addEthTx(btcTxHash, outputIndex, ethTxHash) {
    ...
    if (parsedTxHash !== btcTxHash) {
      throw new Errors.BadRequest(
        `parsed txHash ${parsedTxHash} doesn't match.`
      );
    }
    ...
    if (parsedOutputIndex !== parseInt(outputIndex)) {
      throw new Errors.BadRequest(
        `parsed outIndex ${parsedOutputIndex} doesn't match.`
      );
    }
    const parsedValue = ethers.utils.bigNumberify(
      `0x${dataBuf.slice(0, 32).toString("hex")}`
    );
    ...
    const parsedAccount = receipt.logs[3].topics[2].replace(
      "000000000000000000000000",
      ""
    );
    if (parsedAccount.toLowerCase() !== entry.account) {
      throw new Errors.BadRequest(
        `parsed accountAddr ${parsedAccount} doesn't match.`
      );
    }
